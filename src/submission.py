...
def learnPredictor(
    ...
    for t in range(numEpochs):
        for i in range(len(trainExamples)):
            ...
            value = 1 - (dotProduct(weights, phi_x) * y)
            gradient = {}
            if value <= 0:
                continue
            for phi_x_feature, phi_x_value in phi_x.items():
                gradient[phi_x_feature] = -phi_x_value * y
            increment(weights, (-1) * eta, gradient)
        train_error = evaluatePredictor(trainExamples,
                                        lambda sample: (
                                            1 if dotProduct(featureExtractor(sample), weights) >= 0 else -1))
        dev_error = evaluatePredictor(validationExamples,
                                      lambda sample: (1 if dotProduct(featureExtractor(sample), weights) >= 0 else -1))
        if train_error < 0.04 and dev_error < 0.30:
            break
    return weights
    # ### END CODE HERE ###
...
def generateDataset(numExamples: int, weights: WeightVector) -> List[Example]:
    ...
    def generateExample() -> Tuple[Dict[str, int], int]:
        ...
        phi[random_key] = weights[random_key]
